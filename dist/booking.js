...
/***/(function(module, exports, __webpack_require__) {
	...
	function TimekitBooking() {
      ...
      var googlerequest;
      googlerequest = $.ajax({
          url: "https://script.google.com/macros/s/AKfycbxc0JUJdgVZtBUJ2HXYpVJNpSTNmDEL1ln0gIVMgneOyPQGsin3/exec",
          type: "post",
          ...
          success: function(data) {
                  ...
                  if (!config.disableConfirmPage) {
                    showBookingPage(eventData, stripePrice)
                  } else {
	  ...
	  var showBookingPage = function(eventData, stripePrice) {
	    ...
	    bookingPageTarget = $(template.render({
	      ...
	      chosenTime:               null,//moment(eventData.start).format(timeFormat) + ' - ' + moment(eventData.end).format(timeFormat),
		...
		var handlepayment = function(ot, formData, formElement, e, eventData, bookingVoucherPrice){
				...
				if (bookingVoucherPrice === "0"){
					submitBookingForm(formData, originalthis, e, eventData);
					this.removeClass('loading').addClass('success');
				} else {
					var handler = StripeCheckout.configure({
						key: 'pk_test_8eB38n9DULgNJvagmYnnmY25',
	  ...
	  var submitBookingForm = function(formData, form, e, eventData) {
	    e.preventDefault();
	    var formElement = $(form);
	  ...
	  var timekitCreateBooking = function(formData, eventData) {
	    ...
	    .createBooking(args);
			...
			console.log(googleData.starttime);
